...
getEnvConfig = function () {
  ...
  const host = process.env.mygrate_host
  if (!host) {
    console.error(
      'Configuration not found in migrations/config.json OR environment variables'
    )
    process.exit(1)
  }
...
writeConfig = function (config) {
  return Fs.writeFileSync(
    Path.join(Commands.migrationsDir, 'config.json'),
...
dbInterface = function () {
  ...
  return {
    ...
    minHookDate:
      (_ref = (_ref1 = config.mygrate) != null ? _ref1.minHookDate : void 0) !=
      null
        ? _ref
        : DEFAULT_MINHOOK_DATE,
...
migrationFile = function (version, which) {
  ...
  filename = Path.resolve(
    Commands.migrationsDir + '/' + version + '/' + which + '.sql'
  )
  if (Fs.existsSync(filename)) {
    return filename
  }
  return Path.resolve(
    Commands.migrationsDir + '/' + version + '/' + which + '.js'
  )
}
...
down = function (schema, migrations, cb) {
  ...
  migrate = function (version, cb) {
    var execDownScript, filename, pre, unrecord
    filename = migrationFile(version, 'down')
    pre = ''
    execDownScript = function (cb) {
      if (Fs.existsSync(filename)) {
    ...
    return async.series([execDownScript, unrecord], cb)
  }
...
Commands = {
  ...
  generate: (function (_this) {
    return function (argv, vendor) {
      ...
      if (!Fs.existsSync(Path.resolve(Commands.migrationsBasename))) {
        console.error(
          'ERROR ' +
            Commands.migrationBasename +
            ' directory not found. Try `mygrate init`'
        )
        process.exit(1)
      }
      ...
      if (!Fs.existsSync(path)) {
        Wrench.copyDirSyncRecursive(
          Path.resolve(
            __dirname,
            '../templates/' + vendor + '/content/' + template
          ),
          Path.resolve(path)
        )
        if (template !== 'default') {
  ...
  migrateUp: (function (_this) {
    return function (argv, cb) {
      ...
      ;(_ref = dbInterface()),
        (schema = _ref.schema),
        (config = _ref.config),
        (minHookDate = _ref.minHookDate)
      dirs = null
      lastMigration = null
      migrationsInDb = null
      relMigrationsDir =
        './' + Path.relative(process.cwd(), Commands.migrationsDir)
          ...
          validateMigrations: function (cb) {
            ...
            if (lastMigration != null) {
              ...
              for (_i = 0, _len = migrationsInDb.length; _i < _len; _i++) {
                ...
                if (dirs.indexOf(m.version) < 0) {
                  console.error(
                    "ERROR '" +
                      m.version +
                      "' was migrated in DB but does not exist locally.\n./" +
                      Path.relative(process.cwd(), Commands.migrationsDir) +
                      ' dir might be out of sync.'
                  )
                  missing = 1
                }
              }
              for (_j = 0, _len1 = dirs.length; _j < _len1; _j++) {
                ...
                if (
                  d < last &&
                  !_.find(migrationsInDb, function (m) {
                    return m.version === d
                  })
                ) {
                  console.error(
                    'ERROR ' +
                      ...
                      '/' +
                      d +
                      " will be skipped since it is older than the\nlast migration '" +
                      last +
                      "' in database. Rename the dir if it needs to be migrated."
                  )
                  missing = true
                }
              }
                ...
                return async.series(
                  {
                    ...
                    upscript: function (cb) {
                      ...
                      if (ext === '.sql') {
                        ...
                        return cb('Unhandled migration file: ' + filename)
                      }
                    },
                    recordMigration: function (cb) {
                      var up
                      ...
                      return schema.add(version, up, down, function (err) {
                        ...
                        console.log(
                          'Up ' + Commands.migrationsBasename + '/' + version
                        )
                        return cb(null)
                      })
                    },
          ...
          registerSprocs: function (cb) {
            ...
            return ps.parseDirPattern(sprocsDir, function (err, sprocs) {
              ...
              return async.eachSeries(
                sprocs,
                schema.registerSproc.bind(schema),
                cb
              )
            })
          },
  ...
  exec: (function (_this) {
    return function (lambda, options, cb) {
      var begin, commit, db, execLambda, finaly, series, tx
      ...
      if (options.transaction) {
        ...
        execLambda = function (cb) {
          return async.series(lambda(tx), cb)
        }
        commit = function (cb) {
        ...
        execLambda = function (cb) {
          return async.series(lambda(db), cb)
        }
        series = [execLambda]
        finaly = function (err) {
          return cb(err)
      ...
      return async.series(series, finaly)
    }
  })(this),
}
